...
dfSentiment = pd.read_csv('/Users/gorjantodorovski/dev/mlops-course/Assignments/Week_1-Open_Source_ML_POC_and_App_Builds/Phase_I-Proof_of_concept/sentiment_data.csv')
dfSentiment['timestamp'] = [datetime.strptime(dt, '%Y-%m-%d') for dt in dfSentiment['timestamp'].tolist()]

print(dfSentiment.columns)

# Multi-select columns to build chart
col_list = ['timestamp','sentiment_score', 'close_lag1', 'perc_change_close',
       'sentiment_score_lag1', 'perc_change_sentiment', 'sentiment_SMA3mo']

r_sentiment = re.compile(".*sentiment")
sentiment_cols = ['sentiment_score', 'sentiment_score_lag1','perc_change_sentiment','sentiment_SMA3mo']

r_post = re.compile(".*post")
post_list = ['perc_change_close','perc_change_sentiment']
...
stocks_cols = close_list + volume_list + post_list

# Config for page
st.set_page_config(
    page_title= 'TSLA Stock vs. Reddit Sentiment',
    page_icon='✅',
    layout='wide',
)
...
with st.sidebar:
    ...
    sentiment_select = "perc_change_sentiment"
    stock_select = "perc_change_close"
...
# dashboard title
st.title('TSLA Sentinment Dashboard')
